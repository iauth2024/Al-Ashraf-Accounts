...
def scan_receipt(request):
    if request.method == 'POST' and 'receipt_image' in request.FILES:
        try:
            ...
            # Perform OCR
            extracted_text = pytesseract.image_to_string(image, lang='eng+urd', config='--psm 6')

            # Extract fields using regex or manual matching
            manual_receipt_no = re.search(r'Receipt No[:.]?\s*(\d+)', extracted_text)
            manual_book_no = re.search(r'Book No[:.]?\s*(\d+)', extracted_text)
            amount = re.search(r'₹[:.]?\s*(\d+)', extracted_text)
            name = re.search(r'Received from[:.]?\s*(.+)', extracted_text)
            cheque_number = re.search(r'Cheque[:.]?\s*(\d+)', extracted_text)
            receipt_date = re.search(r'Date[:.]?\s*(\d{2}/\d{2}/\d{4})', extracted_text)

            # Parse fields
            data = {
                'manual_receipt_no': manual_receipt_no.group(1) if manual_receipt_no else None,
                'manual_book_no': manual_book_no.group(1) if manual_book_no else None,
                'amount': int(amount.group(1)) if amount else None,
                'name': name.group(1).strip() if name else None,
                'cheque_number': cheque_number.group(1) if cheque_number else None,
                'receipt_date': datetime.strptime(receipt_date.group(1), '%d/%m/%Y') if receipt_date else None,
            }

            # Save to database or process further
            return JsonResponse({'success': True, 'message': 'Receipt scanned successfully!', 'data': data})
        except Exception as e:
